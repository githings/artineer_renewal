  ...
  <body>
    ...
    <div class="wrap">
      ...
      <div id="container">
        ...
        $query = "select count(*) from project where $where";
        ...
        <div class="container_position">
          ...
          <div class="gallery">
            <?php
            $query = "select * from project where $where order by no desc limit $page, $view_total";
            $result = mysqli_query($connect, $query);
            while($data = mysqli_fetch_array($result)) {
            ?>
            <!--1개 사진-->
            <div class="gallery-item">
              <a href="./view.php?no=<?=$data['no']?>&id=<?=$data['id']?>&bbsname=project">
              <div class="gallery-item-image">
                <div class="gallery-item-image-control">
                  <?php if(!$data['file']) {?>
                    <a href="./view.php?no=<?=$data['no']?>&id=<?=$data['id']?>&bbsname=project"><img src="./image/no_image.jpg"></a>
                    <?php } else {
                      $files = explode(",", $data['file']);
                      ?>
                    <a href="./view.php?no=<?=$data['no']?>&id=<?=$data['id']?>&bbsname=project"><img src="./data/<?=$files[0]?>"></a>
                  <?php }?>
                </div>
              </div>
              <div class="gallery-item-description">
                <table class="sub_news" cellspacing="0">
                  <tr>
                    <th>제목</th>
                    <td colspan="3"><a href="./view.php?no=<?=$data['no']?>&id=<?=$data['id']?>&bbsname=project"><?=mb_substr($data['subject'], 0, 11, 'UTF-8');?> <font color="#20cbd3">(<?=$data['comment']?>)</font>
                      <?php
          ...
          ?>
          <button class="table_write_btn" type="button" onclick="location.href='./write.php?no=<?=$data['no']+1?>&id=<?=$member['id']?>&bbsname=project'">글쓰기</button>
