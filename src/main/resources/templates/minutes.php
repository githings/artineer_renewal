  ...
  <body>
    ...
    <div class="wrap">
      ...
      <div id="container">
        ...
        $query = "select count(*) from minutes where $where";
        ...
        <div class="container_position">
          <div class="containter_table">
            ...
            <table class="sub_news" border="1" cellspacing="0" summary="게시판의 글제목 리스트">
              ...
              <tbody>
                <?php
                $query = "select * from minutes where $where order by no desc limit $page, $view_total";
                $result = mysqli_query($connect, $query);
                ...
                <tr>
                  <td><?=$data['no']?></td>
                  <td class="title">
                    <a href="./view.php?no=<?=$data['no']?>&id=<?=$data['id']?>&bbsname=minutes"><?=mb_substr($data['subject'], 0, 35, 'UTF-8');?> <font color="#20cbd3">(<?=$data['comment']?>)</font>
                      <?php
            ...
            ?>
            <button class="table_write_btn" type="button" onclick="location.href='./write.php?no=<?=$data['no']+1?>&id=<?=$member['id']?>&bbsname=minutes'">글쓰기</button>
